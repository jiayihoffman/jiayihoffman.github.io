...
To start the docker container: <br>
...
We need to get the driver code for RPLidar to scan and publish scan data. The RPLidar has a [git repository](https://github.com/Slamtec/rplidar_ros/tree/ros2) for its ROS 2 ecosystem.  We need to clone the repository and build it using `colcon build`. The RPLidar package provides a node that publishes the scan data to the "/scan" topic, and services to control start and stop the RPLidar motor. Building from the ROS package source code is a common way to consume third-party robotic hardware.

The RPLidar uses a serial port to communicate with ROS. The port is specified when starting the RPLidar node. To find out which USB port is used by the RPLidar, let's connect it to the Pi and issue this command: 
```
$ ls -l /dev/ttyUSB*
crw-rw---- 1 root dialout 188, 0 Feb 10 17:47 /dev/ttyUSB0
```
From this output, it says:
...
```
Then we add the following lines to download the source code for the RPLidar driver:
```
# download the RPLidar code
...
A straightforward approach is to use the `--device` option, formatted as follows: `docker run -it --user ros --network=host --ipc=host --device=/dev/ttyUSB0 my_ros2_image`. However, over time, I found this is not the most convenient approach, as the USB port can change after reconnecting the RPLidar's USB cable. Therefore, a better method is to map to all devices and then use device group rules to restrict it to only USB serial converters, which belong to device group 188, as mentioned in the previous section. 
```
docker run -it --user ros --network=host --ipc=host -v /dev:/dev --device-cgroup-rule='c 188:* rmw' my_ros2_image
```
...
To stop the RPLidar motor, use the command `ros2 service call /stop_motor std_srvs/srv/Empty`. To start the motor, issue the command `ros2 service call /start_motor std_srvs/srv/Empty`.

## Viewing the Lidar Scan

ROS 2 on the Raspberry Pi serves as the robot controller. To keep it lightweight, it features only a command interface and lacks a graphical interface. To monitor the robot's activities, we use a separate system, typically an Ubuntu Linux desktop, which has a graphical user interface and subscribes to the topics published by the robot.
